...
<p style="padding-left:5px"><input name="VoteColor" class="CheckStyle" id="Check_VoteColor_ID" type="checkbox"><label  name="VoteColor" for="Check_VoteColor_ID"><span class="ui"></span>Couleur Vote</label></p>\
    ...
    // Ouberture/Fermeture du menu
    document.getElementById('btnScript').addEventListener('click', function() {Ouverture('MenuScript');});
    ...
    // Change les variables pérmanentes
...
padding-left:200px;\
    ...
    if (!($.browser == "msie" && $.browser.version < 7)) {
        var target = "#leftcolumn", top = $(target).offset().top - parseFloat($(target).css("margin-top").replace(/auto/, 0));
        $(window).scroll(function(event) {
            if (top <= $(this).scrollTop()) {
                $(target).addClass("fixed");
				$("#centercolumn").addClass("fixed");
            } else {
                $(target).removeClass("fixed");
				$("#centercolumn").removeClass("fixed");
...
*/
    ...
    if(bMessage)
    {
        ...
        if (Bold != null && Italic != null && Underline != null && Linethrough != null)
        {
            ...
            Bold.addEventListener('click', function() {AddQuote("[b]", "[/b]");});

            Italic.removeAttribute("onclick");
            Italic.addEventListener('click', function() {AddQuote("[i]", "[/i]");});

            Underline.removeAttribute("onclick");
            Underline.addEventListener('click', function() {AddQuote("[u]", "[/u]");});

            Linethrough.removeAttribute("onclick");
            Linethrough.addEventListener('click', function() {AddQuote("[d]", "[/d]");});
    ...
    for ( i=0;i< ArrowUp.length;i++)
    {
        ...
        VoteID.addEventListener("click",  function() {ShowVote(this);});
        VoteID.addEventListener("mouseout",  function() {HideVote();});
